...
def vocab_config(vocab_config, mode, glove_path=None, result_path=None):
    config = _HParams()
    ...
    if config.mode == 'fixed' and mode != "eval":
        ...
        config.vocab_file = os.path.join(result_path, vocab_config['vocab_file'])
        config.embs_file = os.path.join(result_path, vocab_config['embs_file'])
    return config


def model_config(mdl_config, mode, glove_path=None, result_path=None):
    config = _HParams()
    ...
    if mdl_config['checkpoint_path']:
        config.checkpoint_path = os.path.join(result_path, mdl_config['checkpoint_path'])
    config.vocab_configs = []
    for vocab_configs in mdl_config['vocab_configs']:
        config.vocab_configs.append(vocab_config(vocab_configs, mode, glove_path, result_path))
